...
// Join group
groupRouter.post("/join/:groupId", protect, async (req, res) => {
  try {
...
// Leave group
groupRouter.post("/leave/:groupId", protect, async (req, res) => {
  try {
    const group = await Group.findById(req.params.groupId);
    if (!group) {
      return res.status(404).json({ message: "Group not found" });
    ...
    await group.save();
    res.status(200).json({ message: "Successfully left from the group" });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
});
